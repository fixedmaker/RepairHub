      ...
      <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-4 overflow-hidden">
        {loading ? (
          ...
          <DeviceTable 
            ...
            onEdit={role === 'admin' ? handleOpenEdit : undefined}
          />
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm">
          ...
          >
            ...
            <form onSubmit={handleCreate} className="space-y-4 max-h-[70vh] overflow-y-auto px-1">
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase ml-1">Tugaskan Teknisi (Opsional)</label>
                <select 
                  className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-3 px-4 outline-none focus:ring-2 focus:ring-red-500"
          ...
          >
            ...
            <form onSubmit={handleUpdate} className="space-y-4 max-h-[70vh] overflow-y-auto px-1">
              <div className="space-y-1">
                ...
                <select 
                  required
                  className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-3 px-4 outline-none focus:ring-2 focus:ring-red-500 font-sans"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase ml-1 font-sans">Status Progres</label>
                  <select 
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-3 px-4 outline-none focus:ring-2 focus:ring-red-500 font-sans"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase ml-1 font-sans">Tugaskan Teknisi</label>
                  <select 
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-3 px-4 outline-none focus:ring-2 focus:ring-red-500 font-sans"
              ...
              <div className="flex gap-3 pt-6">
                <button type="button" onClick={() => setEditingDevice(null)} className="flex-1 py-3 bg-slate-100 text-slate-600 rounded-2xl font-bold font-sans">Batal</button>
                <button type="submit" className="flex-1 py-3 bg-repair-gradient text-white rounded-2xl font-bold shadow-lg shadow-red-200 font-sans">Simpan Perubahan</button>
              </div>
            </form>
          </motion.div>
        </div>
      ...
      {viewingDevice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm">
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-6 font-sans">Detail Unit Perbaikan</h2>
            
            <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
              <div className="p-4 bg-slate-50 rounded-2xl space-y-2">
                <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-sans">Informasi Pelanggan</div>
                <p className="text-sm font-bold text-slate-800 font-sans">{viewingDevice.customerName}</p>
                <p className="text-xs text-slate-500 font-mono">ID: {viewingDevice.customerId}</p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-slate-50 rounded-2xl">
                  <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-sans">Merk / Model</div>
                  <p className="text-sm font-bold text-slate-800 font-sans">{viewingDevice.brand} - {viewingDevice.model}</p>
                  <p className="text-xs text-slate-500 font-sans">{viewingDevice.type}</p>
                </div>
                <div className="p-4 bg-slate-50 rounded-2xl">
                  <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-sans">Nomor Seri</div>
                  <p className="text-sm font-semibold text-slate-800 font-mono truncate">{viewingDevice.serialNumber || '-'}</p>
                </div>
              </div>

              <div className="p-4 bg-slate-50 rounded-2xl space-y-1">
                <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-sans">Gejala / Kerusakan</div>
                <p className="text-xs text-slate-600 font-sans whitespace-pre-wrap leading-relaxed">{viewingDevice.damageDescription}</p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-slate-50 rounded-2xl">
                  <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-sans">Status Unit</div>
                  <span className={cn(
                ...
                <div className="p-4 bg-slate-50 rounded-2xl">
                  <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-sans">Teknisi Penanggungjawab</div>
                  <p className="text-sm font-bold text-slate-800 font-sans mt-0.5">{viewingDevice.technicianName || 'Belum Ditugaskan'}</p>
                </div>
              </div>

              {viewingDevice.serviceNotes && (
                <div className="p-4 bg-amber-50/50 border border-amber-100 rounded-2xl space-y-1">
                  <div className="text-[10px] text-amber-600 font-bold uppercase tracking-wider font-sans">Catatan Riwayat Servis</div>
                  <p className="text-xs text-amber-800 font-sans whitespace-pre-wrap leading-relaxed">{viewingDevice.serviceNotes}</p>
                </div>
              )}
              ...
              <div className="border-t border-slate-100 pt-4 mt-2 text-left">
                <DocumentationUpload 
                  ...
                  readOnly={role !== 'admin' && role !== 'technician'}
                />
              </div>
            </div>
